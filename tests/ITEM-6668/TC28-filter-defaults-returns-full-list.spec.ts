...
test.describe('TC28 — Filter with default values returns the full employee list', () => {
  ...
  test('TC28: clicking Filter with all defaults returns a non-empty employee list', async () => {
    ...
    // Step 4: Click the filter button without changing any filter values
    await report.clickFilterButton();

    // Step 5: Wait for search results to load
    await report.waitForSearchResults();

    // Step 6: Expect at least one result in the available items list
