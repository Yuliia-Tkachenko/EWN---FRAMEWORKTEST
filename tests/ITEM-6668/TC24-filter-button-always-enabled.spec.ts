...
test.describe('TC24 — Filter button is always enabled and produces results on click', () => {
  ...
  test('TC24: filter button is enabled with default filters and returns results when clicked', async () => {
    ...
    // Step 4: Expect the filter button to be enabled before any interaction
    await report.expectFilterButtonEnabled();

    // Step 5: Click the filter button with default filter values
    await report.clickFilterButton();

    // Step 6: Wait for search results to load
    await report.waitForSearchResults();

    // Step 7: Expect at least one result in the available items list
